...
for k,v in query_hits.items():
    alien = [x for x in v if x[1].startswith(alien_id)]
    domestic = [x for x in v if x[1].startswith(domestic_id)]
    alien_e = [float(x[10]) for x in alien]
    domestic_e = [float(x[10]) for x in domestic]
    ...
    if alien_index >= 0:
        print('\t'.join([k, str(alien_best), str(domestic_best), str(alien_index)]))
    '''
    else:
